...
import { TypedCircularBuffer } from './circular-buffer';

interface ChartData {
  timestamps: TypedCircularBuffer;
  series: TypedCircularBuffer[];  // 3 series per chart
}

export class ChartManager {
  private charts: uPlot[] = [];
  private chartData: ChartData[] = [];
  private readonly bufferCapacity = 1500; // 60 seconds @ 25Hz
   ...
   */
  addDataPoint(chartIndex: number, timestamp: number, values: number[]): void {
  ...
  clear(): void {
    ...
    const emptyData: uPlot.AlignedData = [[0], [0], [0], [0]];
